...
export default function () {
  ...
  function sendConfirmation(port) {
    const host = this.server.address.address.split('.').join(',');
  ...
  let pasvHost = this.server.address.address;
  ...
  .on('connection', (socket) => {
    if (this._socket.remoteAddress !== socket.remoteAddress) {
      socket.destroy();
      return pasvError.call(this, new Error('Address do not match.'), 550)
      .then(() => this.close());
    }
    this.dataSocket = socket;
    this.dataSocket.setEncoding(this.server.dataEncoding);
    // Should I pause dataSocket and resume in corresponding commands?
    this._socket.resume();
  })
  .on('close', () => {
    this.dataSocket = null;
